...
The Hono worker (`src/worker/index.ts`) exposes three endpoints for AI editing:
- `POST /api/ai-edit/start` — Submits an async job (returns `{ jobId }`), runs in background via `waitUntil`
- `GET /api/ai-edit/status/:jobId` — Polls job state (`pending | complete | error`) and retrieves result
- `POST /api/ai-edit` — Legacy synchronous fallback (same logic, blocks until complete)

The worker translates natural language editing commands into FFmpeg commands using Gemini 2.5 Flash. It does NOT execute FFmpeg — the frontend sends the returned command to the local FFmpeg server's `/process-asset` endpoint.
...
- `POST /session/{id}/remove-dead-air` - Silence-based gap removal (see dedicated section)
- `POST /session/{id}/chapters` - YouTube chapter generation (used by legacy `useVideoSession`)
- `POST /session/{id}/transcribe-and-extract` - Transcription + keyword extraction for B-roll
- `POST /session/{id}/generate-broll` - Auto B-roll placement from keywords
- `POST /session/{id}/analyze-for-animation` - Analyze video/transcript for animation ideas
- `POST /session/{id}/generate-contextual-animation` - Context-aware animation from video content
- `POST /session/{id}/generate-transcript-animation` - Animation from transcript keywords
- `POST /session/{id}/generate-batch-animations` - Multiple animations in one request
- `POST /session/{id}/render-from-concept` - Animation from a text concept description
...
**AI-Generated Dynamic Animations** (`src/remotion/DynamicAnimation.tsx`): Takes `scenes: Scene[]` prop. Composition `id="DynamicAnimation"` is what the FFmpeg server renders. Uses `@remotion/shapes`, `@remotion/animated-emoji`, `@remotion/gif`, `@remotion/lottie`, `@remotion/three`.

Scene `type` values: `'title' | 'steps' | 'features' | 'stats' | 'text' | 'transition' | 'media' | 'chart' | 'countdown' | 'comparison' | 'shapes' | 'emoji' | 'gif' | 'lottie' | '3d'`
...
- **Caption fonts**: Google Fonts preloaded in `index.html` for caption rendering: Bebas Neue, Inter, Montserrat, Oswald, Poppins, Roboto (weights 400/700/900).
...
## Render Pipeline (V2/V3 Overlays)

`handleProjectRender` in `scripts/local-ffmpeg-server.js` builds one `filter_complex` for all clips. Critical rules — **do not break these**:

1. **PTS shift is mandatory:** Every clip uses `setpts=PTS-STARTPTS+(${clip.start}/TB)`. Without this, overlay frames start at PTS=0 but the overlay `enable` expression fires at `t=clip.start`, so FFmpeg finds no frame. When `clip.start=0` the shift is a no-op.

2. **Image/GIF input flags:** Static images need `-loop 1` before `-i` (otherwise 1 frame = 1/30s of content). GIFs need `-ignore_loop 0` before `-i` (most web GIFs have loop_count=0, so this loops them indefinitely).

3. **Image outPoint fallback:** `asset.duration` is 0 for images. Use `clip.outPoint || clip.duration || asset.duration` so `trim=0:clipDuration` works.

4. **V1 sorts before V2 before V3** via `trackOrder`. Overlays chain serially: `[base]→[out0]→[out1]→...→[vout]`.

5. **Overlay position bug:** `clip.transform?.x || '(W-w)/2'` is falsy when x=0 — use `clip.transform?.x != null ? clip.transform.x : '(W-w)/2'` if clips positioned at x=0 matter.

Verified working: frames extracted from test render confirm animation compositing over base video at correct timestamps.
